...
::: tip 前言
apps/admin 作为示例项目，可能会包含业务项目中没有用到的插件，业务项目中可以删除无用的文件或者路由，不引用即可，没有引用就不会打包。
:::
...
之后选择要打包的项目，回车之后将会执行构建流程，构建打包成功之后，会在对应包的根目录生成 dist 文件夹，里面就是构建打包好的文件

## 旧版浏览器兼容

在对应项目 **.env.production** 内

设置 `VITE_LEGACY=true` 即可打包出兼容旧版浏览器的代码，该操作会导致构建速度变慢，应用性能降低
...
如果你的构建文件很大，可以通过项目内置 [rollup-plugin-visualizer](https://github.com/btd/rollup-plugin-visualizer) 插件进行代码体积分析，从而优化你的代码。
...
选择要分析的项目，运行后会自动打开的页面可以看到具体的体积分布，以分析哪些依赖有问题。
...
可在 路由创建相关逻辑处（`createRouter`） 进行 mode 修改
...
开启 history 模式需要服务器配置，更多的服务器配置详情可以看 [history-mode](https://router.vuejs.org/zh/guide/essentials/history-mode.html)
